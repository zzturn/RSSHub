...
const calculateValue = () => {
    ...
    value = {
        ...
        openai: {
            openai_key: envs.OPENAI_KEY,
            // 获取 openai key 的请求地址
            openai_req_url: envs.OPENAI_REQ_URL,
            // 获取 openai key 的 key，不是那个用来给 openai 发请求的key
            openai_req_key: envs.OPENAI_REQ_KEY,
            model: envs.OPENAI_MODEL || 'gpt-3.5-turbo-16k-0613',
            temperature: envs.OPENAI_TEMPERATURE || 0.2,
            max_tokens: envs.OPENAI_MAX_TOKENS || null,
            url_base: envs.OPENAI_URL_BASE || 'https://api.openai.com/v1',
            prompt:
                envs.OPENAI_PROMPT ||
                '我会提供一篇文章，你需要从中提取出5个关键词，并分析文章脉络用中文归纳出文章的大纲，最后再总结一个综合摘要。摘要应涵盖原始文本中呈现的所有关键点和主要观点，而且要包含支持主要观点的相关细节和示例，同时避免任何不必要的信息或重复内容。摘要长度应适合原始文本的长度和复杂性，提供清晰准确的概述，并不遗漏任何重要信息。你的输出内容还需要满足以下要求：1.分为“关键词”，“大纲”，“总结”这三个段落 2. 以严格的 markdown 格式输出，并加粗段落标题，在大纲中使用多级列表',
        },
    };
};
calculateValue();
