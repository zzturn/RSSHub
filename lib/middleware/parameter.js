...
const summarizeArticle = async (articleText) => {
    const apiUrl = `${config.openai.url_base}/chat/completions`;
    let max_count = 5;
    /* eslint-disable no-await-in-loop */
    while (max_count > 0 && !config.openai.openai_key) {
        config.openai.openai_key = await getOpenaiKey();
        max_count--;
    }
    if (!config.openai.openai_key) {
        logger.debug(`获取 openai key 失败， 已重试 ${5 - max_count} 次，请稍后再试`);
        throw Error(`获取 openai key 失败，已重试 ${5 - max_count} 次，请稍后再试`);
    }
    try {
        ...
        config.openai.openai_key = undefined;
        error.message = `key: ${config.openai.openai_key}\n${error.message}`;
        throw error;
    }
};

const getOpenaiKey = async () => {
    const apiUrl = config.openai.openai_req_url;
    try {
        const response = await got.post(apiUrl, {
            headers: {
                authorization: `Bearer ${config.openai.openai_req_key}`,
            },
        });
        return response.data.token;
    } catch (error) {
        logger.debug(`获取 openai key 失败，错误信息: ${error.message}`);
        return undefined;
    }
};
            ...
            if (ctx.query.gpt) {
                ctx.state.data.item = await Promise.all(
                    ctx.state.data.item.map(async (item) => {
                        if (item.description) {
                            try {
                                const summary = await ctx.cache.tryGet(
                                    `openai:${item.link}`,
                                    async () => {
                                        ...
                                        return marked.parse(summary_md);
                                    },
                                    30 * 24 * 60 * 60 // 30 days
                                );
                                // 将总结结果添加到文章数据中
                                ...
                                logger.debug(`AI 总结失败，错误信息: ${e.message}`);
                                item.description = `AI 总结失败，错误信息: ${e.message} <hr/><br/>' + ${item.description}`;
                            }
                        }
                        return item;
                    })
